...
- Last updated: 2026-08-30 (two marks, two questions. The `new messages` divider and `/history` rest on the **AFK line** (`ChatState::afk_lines`): after `AFK_LINE_IDLE` (10 min) of keyboard silence the room on screen gets a line stamped at the moment the keyboard went quiet, cleared only when a message you wrote lands there, never persisted. A bare `/summary` rests on the **left-app mark**: `Drop for App` writes `user_ssh_keys.left_at` (migration 166) at the moment the keyboard went quiet before the session ended, the next session on that SSH key takes it (`UserSshKey::take_left_at` nulls the column, so a lost write degrades to the 24h default rather than a stale window), and `ChatState::device_left_at` holds it for the session; the window is always "since you last left the app on this device", capped at 48h and the overlay head says when the cap bit. The two never feed each other: the AFK line is not a `/summary` input and `left_at` seeds no divider. Both stop sharing `chat_room_members.last_read_at`, which records presence rather than reading and so could answer neither; it still drives rail badges and nothing else. `RoomTailLoaded` carries no read cursor at all now, which takes three defects with it: a divider frozen at the last tail load, one device's room-open rewriting another's divider through the per-user event fan-out, and a tail racing the debounced cursor flush. The per-account `chat_summary_reads` watermark tried on this branch is gone, table and all. `late-ssh/src/app/chat/CONTEXT.md` §The Two Marks and §14 Summary. Previous: the pot leaves the sidebar. `RightSidebarComponent::Pot` and `pot/panel.rs` are gone, variant and all, so the rail is lobby, music, and bonsai; a stored `"pot"` panel key is dropped on read by `from_key` like the retired `"activity"` and `"visualizer"` keys, which is why no settings migration is needed. The size and the countdown still ride the status HUD badge off the same `App.pot_view`, and `/pot` answers in full, so nothing about the raffle itself moved. In the same pass the `/summary` catch-up dates its window on the reader's clock: the overlay head reads `3 messages since Aug 28 16:30 CEST` when the account has a timezone and keeps `... 14:30 UTC` when it does not, through `common/time.rs::instant_for_viewer` and a `viewer_tz` mirror synced onto `ChatState` by `App::tick` like the translate settings. `late-ssh/src/app/pot/CONTEXT.md`. Previous: the round, the sink whose whole product is a #lounge line: telling @bartender "round for everyone" buys a drink for every human online but you, `ROUND_PRICE_PER_PATRON` (100) a head through `ChipMove::RoundPurchase`, floor-guarded and burned whole like the crown, out of Top Chips. The trigger is a literal phrase from the closed `ROUND_PHRASES` list (`late-core/src/models/drink_round.rs`, migration 164), never a model decision: it is the only bartender action that spends more than one drink's worth, the price is the size of the room, so the phrase is the confirmation, and `chat/slur.rs` reads the same list to leave it unscrambled (drunk text is stored, not rendered, so "ronud for eevryone" would break the feature for the patrons most likely to use it). Only the buyer is poured into (on the spot, in the purchase transaction); everyone else, because a drink makes someone type drunk in public, gets a `drink_credits` row cashed only by ordering from @bartender, one open per patron across every round (a partial unique index, which is also why a second round moments later reaches nobody and refuses uncharged), 24h to claim, worth a flat `ROUND_DRINK_POINTS` (300), exactly the buzzed threshold. The buyer pays for the credits the grant's own `RETURNING` produced, in the same transaction, so what was charged and what was poured cannot disagree. Presence is the in-process `active_users` roster minus bots and the buyer, single-replica on purpose (SHOP.md Phase 8 status): there is no presence table in this repo, `users.last_seen` is written only at connect. `late-ssh/src/app/chat/CONTEXT.md` §9d. Previous: sharing news pays. Publishing an article credits the sharer `NEWS_SHARE_REWARD_CHIPS` (500) through `ChipMove::NewsShared`, and it pays the same from the News composer as from `s` on an RSS entry, because both funnel into `ArticleService::do_process_url`. `Article::create_shared` (`late-core/src/models/article.rs`) is the only path a user-facing share may take: in one advisory-locked transaction it inserts the row, then credits only if this user has no `news_shared` ledger row for this URL and fewer than `NEWS_SHARE_MAX_PAID_PER_DAY` (3) of them dated today UTC (migration 163 indexes that lookup); the closed `NewsShareReward` it returns drives the banner and the metric. The ledger, not the `articles` row, is the record of payment, because deleting a story frees its URL to be shared again and paying on insert alone would let one player share, delete, and re-share the same link forever. `articles.url` is unique, so a link only ever pays its first sharer while it is live; the cap is per person, not per link. Reward counts toward Top Chips like any other earning. `late-ssh/src/app/chat/CONTEXT.md` §11 News. Previous: the pot, the sink that works at any concurrency and ships one story a week: a parimutuel raffle drawn Monday 21:00 UTC, tickets 100 chips, at most 10 per player per UTC day (`late-core/src/models/pot.rs`, migration 160). At the draw one ticket is pulled weighted by holding, its owner takes 80% of what the tickets paid in (`ChipMove::PotWon`) and the other fifth is never re-minted; nobody in the pot means it rolls and pays no one. The next pot opens in the same transaction, so exactly one is always open. Neither `PotTicket` nor `PotWon` counts toward Top Chips: a lottery must not top the earners board, and excluding only the win would make buying in a pure negative. Every replica sweeps; a status transition (`UPDATE ... WHERE status = 'open' RETURNING *`) under an advisory lock is what makes exactly one of them pay, and the winner's banner rides the `pot_changed` notify so it reaches them wherever they are connected. The status HUD badge (there was a two-row sidebar panel until 2026-08-28) and `/pot`, `/pot buy N`. `late-ssh/src/app/pot/CONTEXT.md`.) Previously 2026-08-26 (repeatable door payouts, the other half of the sink story: every door milestone pays again, at the amounts in SHOP.md's Phase 6 table (migration 158), with the gate set by whatever naturally limits the game. Roguelikes pay per ingested log line behind a 7-day per-milestone lockout, Lateania's crowns once per `mud_characters.id` behind the same lockout, Green Dragon for every kill (the kill resets the character) and A Dark Room for every run that gets out (the ending wipes the save, and `Game.run_id` in the blob is the key). The two-gate payouts ride one new primitive, `GamePayout::grant_multi`: several claim rows and the credit in one advisory-locked transaction, all-or-nothing, with legacy `lifetime` rows invisible to every new gate. Profile badges are untouched and still land once per account. `late-ssh/src/app/leaderboard/CONTEXT.md`.) Previously 2026-08-26 (burn milestones, the top of the Shop: three permanent badges at 50,000 / 150,000 / 500,000 (`milestone_badge`, migration 157) whose whole product is the receipt, worn on top of a rented badge and flag so nothing cheap can hide one, with the dearest owned rung showing and no equip flow anywhere. The same migration drops both ultimate spells from 10,000,000 to 1,000,000, making 1M the ceiling of the Shop. `late-ssh/src/app/hub/CONTEXT.md`.) Previously 2026-08-26 (the crown, the Shop's second sink outside the Shop and the first one nobody prices: one slot, one holder, one 👑 after their name in every chat author header and on the Clubhouse floor. `/crown` prints who wears it and what taking it costs; `/crown take` buys it for `max(500, ceil(paid x 1.5))` (`late-core/src/models/crown.rs`, migration 156), so the price ratchets with whoever last paid and we never tune it. Every chip is destroyed: `ChipMove::CrownTaken` is a debit with no matching credit reason anywhere. There is no hold or cooldown (the 1.5x ladder is the throttle, and month end is a real auction), you cannot take a crown you already wear, and the slot empties at the UTC month rollover with no sweeper (a reign is current only while its month is), leaving the month's last holder the permanent `CRWN` profile award. The glyph rides the once-a-second `name_flair` resolution off a process-shared holder kept in step by the `crown_changed` Postgres notify, so no render ever queries for it. Every takeover posts to #lounge naming both players. `late-ssh/src/app/chat/CONTEXT.md` §9c.)
- Status: Active
...
| `late-ssh/src/app/pot/CONTEXT.md` | The weekly pot: `/pot` and `/pot buy N`, `PotService`, the `pots` / `pot_tickets` tables, the status HUD badge, the draw sweeper, or the pot's #lounge lines. | Pot domain context: the fixed numbers (100-chip tickets, 10 per player per UTC day, 80% payout, Monday 21:00 UTC), the two `ChipMove` variants and why neither counts as earnings, the three concurrency guards (partial unique index, advisory lock, status transition), the `pot_changed` distribution model and the private holdings map, the status HUD badge (there is no sidebar panel any more), the draw's feed line and headline, telemetry, and the gotchas. |
...
| LeWordGame | `le_word_games` | `UNIQUE(user_id, mode)`, one dated daily slot plus one undated random-word replay slot; stores answer, guesses, and game-over/win flags |
...
| Pot | `pots` | The weekly raffle (migration 160). `status` IN (`open`, `drawn`, `rolled`) with a partial unique index on `(status = 'open')` so at most one is open ever; `draws_at` is the next Monday 21:00 UTC, `ticket_price` 100. `winner_user_id`/`ticket_count`/`payout_chips`/`drawn_at` are all NULL while open and stamped once at the draw, enforced by one CHECK per status (a drawn pot is told from a rolled one by `ticket_count > 0`, not by the winner, whose account can be deleted out from under a settled row). Migration 162 dropped `announced_threshold` with the threshold lines. |
...
- **Every top-level screen change ends in `force_full_repaint`.** `App::set_screen` (`app/state.rs`) calls it once after assigning `self.screen`, replacing the per-screen calls the doors, Artboard, Lateania, Rebels, DailyMatch, HouseTable and Scratchpad each made on the way out. Two layouts rarely disagree on every cell, and a wide glyph the new layout only half-overwrites survives ratatui's diff: leaving Home for the Arcade used to strand the sidebar's bonsai emoji on screen. The cost is one full frame per page switch, which most transitions already paid. A same-screen `set_screen` call (pressing the nav key for the page already open) changes no layout and skips the clear.
- **SSH compression is disabled server-side.** `russh::server::Config.preferred.compression` is pinned to `[compression::NONE]` in `ssh.rs`; russh otherwise advertises zlib by default, and clients that negotiated it (`ssh -C`, `Compression yes`) were dropping mid-session with "closed by remote host" on the first keystroke that moved a character. A TUI frame diff is small and already poorly compressible, so there is nothing to win by re-enabling it.
